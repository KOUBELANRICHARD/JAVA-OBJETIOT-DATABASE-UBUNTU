...
void loop() {
  temp = dht.readTemperature();    // Lecture de la température depuis le capteur DHT11
  humid = dht.readHumidite();                   // Affichage de la température dans le moniteur série

  // Création du payload (contenu du message) à envoyer au serveur
  String payload = "{\"code\": \"FP78\"},\"temperature\": " + String(temp) +" ,\"humidite\": " + String(humidite)";
